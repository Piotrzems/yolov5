...
class Annotator:
    ...
    def point_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            self.draw.point(point, fill = (128, 128, 128))
            LOGGER.info(f'point drawn')
            if label:
            ...
            point = (int(box[2]), int(box[3]))
            #cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
            cv2.circle(self.im,point, 5, (0,0,255), -1)
            #LOGGER.info(f'circle added')
            if label:
